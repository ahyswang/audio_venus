...
static void vec_normf(void* y, void* x, unsigned N)
{
    float* p_x = (float *)x;
    float* p_y = (float *)y;
    float mean = 0.0f;
...
int snoring_detect_process(snoring_detect_t *svr, void* wave, void* classes, unsigned num_samples)
{
    ...
    ASSERT((num_samples >= p_conf->frame_shift) &&
        (num_samples <= p_conf->frame_shift*p_conf->max_frame_num) && 
        (num_samples%p_conf->frame_shift == 0));

    num_frames = num_samples / p_conf->frame_shift;
    // compute fbank
    num_frames_2 = fbank_compute(svr->p_fbank, p_wave, svr->p_fea, num_samples);
    // mean/istd    
    vec_normf(svr->p_fea, svr->p_fea, num_frames_2*p_conf->num_bins);
    if (p_conf->extra1) {
        memcpy(p_conf->extra1, svr->p_fea, num_frames_2*p_conf->num_bins*sizeof(float));
    }
    if (p_conf->extra2) {
        memcpy(svr->p_fea, p_conf->extra2, num_frames_2*p_conf->num_bins*sizeof(float));
    }
    // backbone forward
    thinker_process(svr, p_conf, num_frames_2);
    // classifier
